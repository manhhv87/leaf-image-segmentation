...
def generate_background_marker(image_file):
    """
...
def segment_leaf(image_file, fill_mode, smooth_boundary, marker_intensity):
    """
    Segments leaf from an image file

    Args:
        image_file (string): full path of an image file
        fill_mode (string {no, flood, threshold, morph}): how holes should be filled in segmented leaf
    ...
    org, mrk = generate_background_marker(image_file)

    # set up binary image for further processing
    bin_image = np.zeros((org.shape[0], org.shape[1]))
    bin_image[mrk] = 255
    bin_image = bin_image.astype(np.uint8)

    # further processing of image, filling holes, smoothing edges
    largest_mask = select_largest_obj(bin_image, fill_mode=fill_mode, smooth_boundary=smooth_boundary)
